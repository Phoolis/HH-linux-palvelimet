...
- Luodaan ssh-avain ensin omalla koneella (eli nyt virtuaali Debianissa) `ssh-keygen -t ed25519`. Protokollana RSA:ta turvallisempi Ed25519.
- Openssh-ohjelma piti tietenkin asentaa ensin (pääkäyttäjän oikeuksilla tietysti, hölmö).
...
-----
Tässä oli pitkä selvitys ssh-avainten käytöstä ja rootin lukitsemisesta kun ssh-avain on käytössä yms. Katosi kun en muistanut commitoida muutoksia selaimessa. Palaan tähän ehkä myöhemmin.
-----

Webbisivu pystyssä osoitteessa `65.109.175.125`. Seuraavalla tunnilla vissiin hommataan sille oma domaini. Mutta nyt loppui aika. Koitan paremmalla ajalla uudelleen kirjoittaa kadonneen osion palvelimen käyttäjien ja ssh-avaimen asetuksista.
